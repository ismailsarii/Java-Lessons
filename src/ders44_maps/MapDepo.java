...
public class MapDepo {
    ...
    // ogrenci map olusturup bize donduren bir metod olusturun
    ...
    public static Map<Integer, String> ogrenciyeYeniSubeAta(Map<Integer, String> ogrenciMap, int ogrenciKey, String yeniSube) {
        ...
        // ogrenciKey map'de var mi ? 104

        if (ogrenciMap.containsKey(ogrenciKey)) {
            // istenen key varsa update edip yeni map'i dödüreceğiz

            String istenenKeyinValuesu = ogrenciMap.get(ogrenciKey); // "Ayse-Can-10-H-TM"
            String[] istenenKeyvaluArr = istenenKeyinValuesu.split("-"); // [Ayse, Can, 10, H, MF]
            // yeni sube degerini atayalim
            istenenKeyvaluArr[3] = yeniSube; // [Ayse, Can, 10, K, MF]

            String istenenKeyinYeniValuesu =istenenKeyvaluArr[0] + "-" +
                                            istenenKeyvaluArr[1] + "-" +
                                            istenenKeyvaluArr[2] + "-" +
                                            istenenKeyvaluArr[3] + "-" +
                                            istenenKeyvaluArr[4];
